...
parser = argparse.ArgumentParser(description='HTML Scanner 1.0')

parser.add_argument('-v', '--version', action='version', version='%(prog)s 1.0')
parser.add_argument('url', type=str, help="The URL of the HTML to analtyze")
    ...
    if(config['forms']):    
     for form in forms:
        if((form.get('action').find('https') < 0) and (urlparse(url).scheme != 'https')):
         report += 'Insecure form ' +  form.get('action') +  ' found in document \n'
...
if(report == ''):
    report += 'Nice job! Yout HTML doucment is secure!'
else:
    header = 'HTML Vulnerability Scanner Report is  as follows:\n'
    header +='===================================\n\n'
